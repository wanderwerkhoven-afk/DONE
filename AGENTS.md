...
2. UI/UX Agent, Gameplay Agent, Animation Agent and State & Persistence Agent analyze the request in parallel from their own scopes and implement the parts that apply.
...
8. Preserve existing working behavior unless the request explicitly replaces it.
9. Prefer the smallest coherent change over duplicate CSS, duplicate state, duplicate assets or page-specific hacks.
10. Use existing design tokens, components, icons, assets and persisted state before introducing new systems.
11. Mobile is first-class. Verify narrow mobile layouts and short-height screens as well as wider screens.
12. Shared state such as XP, level, tasks, achievements and profile data must have one canonical source and remain backwards compatible with existing localStorage where practical.
13. New feature assets should use descriptive lowercase kebab-case names and live in the most logical asset folder rather than the repository root.
14. Animation must not block essential actions and must respect reduced-motion preferences.
15. Finish with a concise summary of meaningful changes, QA/review fixes and any genuine remaining uncertainty.
