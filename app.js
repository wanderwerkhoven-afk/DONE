...
state.theme=state.theme==="light"||state.theme==="dark"
  ?state.theme
  :(state.darkMode===false?"light":"dark");
...
// ============================================================================
const HOME_NIGHT_FIREFLY_POINTS=[
  {x:5.9,y:57.2,size:2.4,glow:10.8,delay:-2.1},
  {x:27.7,y:60.3,size:2.6,glow:11.6,delay:-6.4},
  {x:2.3,y:62.4,size:2.9,glow:12.4,delay:-8.7},
  {x:12.0,y:72.1,size:2.7,glow:10.9,delay:-4.2},
  {x:46.7,y:78.1,size:2.4,glow:12.8,delay:-9.1},
  {x:52.1,y:79.5,size:2.7,glow:11.9,delay:-3.6},
  {x:77.9,y:84.9,size:2.2,glow:13.2,delay:-7.5},
  {x:88.5,y:87.0,size:2.5,glow:12.1,delay:-5.0}
];
const homeNightFireflies=()=>HOME_NIGHT_FIREFLY_POINTS.map((point,index)=>`<i class="hero-firefly" data-firefly-index="${index}" style="--size:${point.size}px;--glow-duration:${point.glow}s;--delay:${point.delay}s"></i>`).join("");
let homeNightHeroMetrics=null;
const positionNightFireflies=()=>{
  const hero=document.querySelector(".hero.hero-image");
  if(!hero)return;
  const dots=[...hero.querySelectorAll(".hero-firefly")];
  if(!dots.length)return;

  const place=(naturalWidth,naturalHeight)=>{
    const rect=hero.getBoundingClientRect();
    if(!rect.width||!rect.height)return;
    const scale=Math.max(rect.width/naturalWidth,rect.height/naturalHeight);
    const renderedWidth=naturalWidth*scale;
    const renderedHeight=naturalHeight*scale;
    const offsetX=(rect.width-renderedWidth)*.5;
    const positionY=window.matchMedia("(max-width:499px)").matches?.60:.57;
    const offsetY=(rect.height-renderedHeight)*positionY;

    dots.forEach((dot,index)=>{
      const point=HOME_NIGHT_FIREFLY_POINTS[index];
      if(!point)return;
      dot.style.left=`${offsetX+(point.x/100)*renderedWidth}px`;
      dot.style.top=`${offsetY+(point.y/100)*renderedHeight}px`;
    });
  };

  if(homeNightHeroMetrics){
    place(homeNightHeroMetrics.width,homeNightHeroMetrics.height);
    return;
  }

  const probe=new Image();
  probe.onload=()=>{
    homeNightHeroMetrics={width:probe.naturalWidth,height:probe.naturalHeight};
    place(probe.naturalWidth,probe.naturalHeight);
  };
  probe.src=HOME_HERO_BY_PERIOD.night;
};
let homeFireflyResizeQueued=false;
window.addEventListener("resize",()=>{
  if(homeFireflyResizeQueued)return;
  homeFireflyResizeQueued=true;
  requestAnimationFrame(()=>{
    homeFireflyResizeQueued=false;
